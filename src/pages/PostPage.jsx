...
export default function PostPage() {
  ...
  if (loading)
    ...
    return (
      <main className="flex flex-col justify-center max-w-[1480px] w-full p-5 mx-auto mt-40">
      <div className="">
      <h1 className="text-3xl lg:text-4xl text-center font-serif w-full mx-auto mt-8 xl:mt-10">
        {post && post.title}
      </h1>
      
      <Link to={`/search?category=${post && post.category}`} className="self-center mt-5">
        <Button color="gray" pill size="xl" className="rounded-2xl py-2 px-8 mx-auto mt-5">
          {post && post.category}
        </Button>
      </Link>
        {isVideo ? (
